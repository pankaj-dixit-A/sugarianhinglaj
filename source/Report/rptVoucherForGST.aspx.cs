...
public partial class Report_rptVoucherForGST : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (dsV.Tables[0].Rows.Count > 0)
            {
                ...
                if (Broker_CODE != "2" || !string.IsNullOrWhiteSpace(Broker_CODE))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(BrokerMail))
                    {
    ...
    protected void dtlDetails_OnItemDataBound(object sender, DataListItemEventArgs e)
    {
        try
        {
            ...
            if (dsV.Tables[0].Rows.Count > 0)
            {
                ...
                //dsV.Tables[0].Columns.Add(new DataColumn("Narration5", typeof(string)));
                ...
                string CarporateSaleNo = clsCommon.getString("Select Carporate_Sale_No from " + tblPrefix + "deliveryorder where doc_no=" + Do_No + " and tran_type='DO' and company_code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and Year_Code=" + Convert.ToInt32(Session["year"].ToString()));

                string PODetails = clsCommon.getString("Select PODETAIL from " + tblPrefix + "CarporateSale where Doc_No=" + CarporateSaleNo + " and Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and Year_Code=" + Convert.ToInt32(Session["year"].ToString()));
    ...
    protected void dtl_OnItemDataBound(object sender, DataListItemEventArgs e)
    {
        ...
        Label lblCompanyGSTNo = (Label)e.Item.FindControl("lblCompanyGSTNo");

        lblCompanyGSTNo.Text = "GSTIN NO:" + clsCommon.getString("Select Gst from Company where Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()));
        ...
        Label lblMillAmount = (Label)e.Item.FindControl("lblMillAmount");
        if (vtype == "OV")
        {
            lblMillAmount.Visible = true;
        }
